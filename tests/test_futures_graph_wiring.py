...
class TestMarkPriceNode:
    def test_market_decision_with_fetch_failure_yields_none(self):
...
class TestGraphSetupFuturesTail:
    ...
    def test_pm_connects_to_futures_tail_when_enabled(self, tmp_path):
        """Crypto-only mode: PM feeds the futures tail unconditionally
        (via Mark Price, which must run before the Risk Gate)."""
        setup = self._build_setup(config={
            "futures_risk_state_path": str(tmp_path / "state.jsonl"),
            "futures_orders_log_path": str(tmp_path / "orders.jsonl"),
        })
